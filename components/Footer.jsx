...
const Footer = () => {
    ...
    const linkSections = [
        ...
        },
        {
            title: "CONTACT",
            links: [
                { text: customize?.contactInfo?.phone || "9960752618", path: '/', icon: PhoneIcon },
                { text: customize?.contactInfo?.email || "binarycomputersndbr@gmail.com", path: '/', icon: MailIcon },
                { text: customize?.contactInfo?.address || "22,1st Foor Saiachana Plaza, Bus Stand, Nandurbar-425412", path: '/', icon: MapPinIcon }
            ]
        }
    ];

    const socialIcons = [
        { icon: FacebookIcon, link: customize?.socialMedia?.facebook || "https://www.facebook.com" },
        { icon: InstagramIcon, link: customize?.socialMedia?.instagram || "https://www.instagram.com" },
        { icon: TwitterIcon, link: customize?.socialMedia?.twitter || "https://twitter.com" },
        { icon: LinkedinIcon, link: customize?.socialMedia?.linkedin || "https://www.linkedin.com" },
    ]
        ...
        <footer className="mx-6 bg-white">
            <div className="max-w-7xl mx-auto">
                <div className="flex flex-col md:flex-row items-start justify-between gap-10 py-10 border-b border-slate-500/30 text-slate-500">
                    <div>
                        <Link href="/">
                            <img src={customize?.logo || "/logo.png"} alt="Binary Computers" className="w-44 h-auto object-contain" />
                        </Link>
                        <p className="max-w-[410px] mt-6 text-sm">Welcome to Binary Computers, your ultimate destination for the latest and smartest gadgets. From smartphones and smartwatches to essential accessories, we bring you the best in innovation — all in one place.</p>
                ...
                <p className="py-4 text-sm text-slate-500">
                    Copyright 2025 © Binary Computers All Right Reserved.
                </p>
            </div>
        </footer>
    );
